...
import { faArrowRight, faCar, faCarOn, faEnvelope, faIdCard, faPaperclip, faStar, faTags, faUsers, IconDefinition } from '@fortawesome/free-solid-svg-icons';
  ...
  return (
    <div className="gap-3 flex-column transaction-list d-flex">
      {transactions.map((transaction) => (
        <div key={transaction.id} className="p-3 bg-white border rounded d-flex align-items-center">
          ...
          <div className="ms-auto text-end">
            <div className="fw-medium">{currencyFormat(parseInt(transaction.amount))}</div>
            <div className="gap-1 d-flex align-items-center small">
              <Badge bg={transaction.status === "Active" ? "success" : "secondary"} className="rounded-pill">
                {transaction.status}
              </Badge>
              <span className="text-muted">{dayjs(transaction.date).format('DD MMM YYYY')}</span>
            </div>
          </div>
        </div>
      ))}
