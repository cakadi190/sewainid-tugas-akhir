...
import { Badge, Button, Card, Col, Collapse, ListGroup, Modal, Row } from "react-bootstrap";
import {
  FaCalendar,
  FaCreditCard,
  FaDownload,
...
const BillingParties = memo(({ transaction }: { transaction: TransactionPageDetailProps['transaction'] }) => (
...
const TransactionInfo = memo(({ transaction }: { transaction: TransactionPageDetailProps['transaction'] }) => {
  const transactionFields = useMemo(() => [
    { label: 'ID Transaksi', value: `#${transaction?.id}` },
    { label: 'Kode Referensi', value: transaction?.payment_references || '-' },
    { label: 'Tanggal Dibuat', value: formatDateToWIB(transaction?.created_at || '') },
    { label: 'Terakhir Diupdate', value: formatDateToWIB(transaction?.updated_at || '') },
  ...
  return (
    <div className="mt-2">
      ...
      <Modal show={modalState} onHide={() => setModalState(false)}>
        ...
        <Modal.Body>
          {transactionDetail?.instructions && transactionDetail.instructions.map((instruction, index) => (
            <div key={instruction.title || index} className="mb-3">
              ...
              >
                {instruction.title}
                <i className={`fas fa-chevron-${openCollapse === `instruction-${index}` ? 'up' : 'down'} float-end`}></i>
              </Button>

              <Collapse in={openCollapse === `instruction-${index}`}>
                <div id={`collapse-${index}`}>
                  <div className="mt-2 card card-body">
                    {instruction.steps && instruction.steps.map((step, stepIndex) => (
        ...
        <div>
          ...
          <div>
            <small className="mb-0">
              Silahkan pindai QR Code disamping dengan aplikasi yang mendukung QRIS Payment.
              Ketuk gambar untuk memperbesar gambar QRIS-nya.
            </small>
          </div>
        </div>
        <img
  ...
  if (transactionDetail?.pay_code) {
    return (
      <div className="mt-4 d-flex justify-content-between align-items-center">
        ...
        <div>
          <Button size="sm" onClick={() => copy(String(transactionDetail.pay_code || '-'))} variant="secondary">
  ...
  return (
    <div className="gap-2 mt-4 d-flex align-items-center justify-content-between">
      <div>
        ...
        <div>
          <small className="mb-0">
            Silahkan klik tombol disamping/dibawah ini untuk melanjutkan pembayaran ke pihak ketiga kami.
          </small>
        </div>
      </div>
  ...
  const fetchChannelName = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        route('v1.global.transaction.get-channels', { search: transaction.payment_channel })
      );
