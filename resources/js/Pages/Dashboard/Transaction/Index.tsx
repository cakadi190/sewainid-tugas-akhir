...
const ExpiredPopover = () => (
  ...
  >
    <div data-bs-toggle="popover" title="Kadaluarsa" data-bs-content="Tagihan ini telah kadaluarsa">
  ...
  const columns: Column[] = [
    ...
    },
    {
      ...
      render: (data: null | TransactionStatusEnum) => (
        <Badge color={getTransactionStatusColor(data as TransactionStatusEnum)}>{getTransactionStatusLabel(data as TransactionStatusEnum)}</Badge>
      ),
    },
    {
      data: 'expired_at',
    ...
    },
    {
      ...
      render: (data: string) => (
        <div>
          <Link style={{ aspectRatio: 1, width: 'fit-content' }} className="btn btn-primary btn-sm d-flex justify-content-center align-items-center" href={route('dashboard.transaction.show', data)}>
            <FaEye />
          </Link>
        </div>
      ),
