...
import { FaCreditCard, FaMapPin, FaPencil } from "react-icons/fa6";
...
interface CheckoutProps {
  carData: Pick<
    Database["CarData"],
    "id" | "car_name" | "brand" | "license_plate" | "rent_price" | "slug"
  >;
  order: {
    car_id: string;
    pickup_date: string;
...
const CheckoutProvider = ({
  ...
  const [order, setOrder] = useState({
    ...
    return_date:
      initialOrder.return_date || dayjs().add(1, "day").format("YYYY-MM-DD"),
  });
  ...
  const setFormData = (
    fieldOrObject: keyof FormData | Partial<FormData>,
    value?: any
  ) => {
...
const DateRangeModal = ({ show, onHide, forbiddenDate }: ModalProps) => {
  ...
  const handleSave = () => {
    if (dateRange.length === 2) {
      ...
      const startDate =
        dateRange[0] instanceof Date && !isNaN(dateRange[0].getTime())
          ? dateRange[0]
          : new Date();

      const endDate =
        dateRange[1] instanceof Date && !isNaN(dateRange[1].getTime())
          ? dateRange[1]
  ...
  const disabledDates = (forbiddenDate || [])
    .map((dateStr) => {
      try {
  ...
  return (
    <Row className="mb-4 g-4">
      <Col md={4}>
        <Image
          src={
            wrapOptimizeUrl(carThumbnail.original_url) ||
      ...
      <Col md={8}>
        ...
        <div className="d-flex justify-content-between">
          <LabelValue
            ...
            value={
              dayjs(order.pickup_date).format("DD MMMM YYYY") +
              " s/d " +
              dayjs(order.return_date).format("DD MMMM YYYY")
            }
            noMarginBottom
            bottomBorder={false}
          />
    ...
    <>
      <Card body className="mb-3 rounded-3">
        <div className="gap-2 pb-3 mb-3 d-flex align-items-center justify-content-between border-bottom">
          ...
          <ButtonEdit type="button" onClick={() => setShowModal(true)}>
            <FaPencil />
          </ButtonEdit>
        </div>
        <div className="d-flex">
          <div className="gap-1 d-flex flex-column">
            <strong className="mb-0">
              {selectedLocation?.name || "Pilih Lokasi Tujuan"}
            </strong>
            <p className="mb-0">
              {selectedLocation?.address ||
                "Klik ikon pensil untuk memilih lokasi tujuan"}
            </p>
          </div>
        </div>
        ...
        <div className="d-flex align-items-center justify-content-between">
          <div className="gap-1 d-flex flex-column">
            {selectedPaymentChannel ? (
              ...
              <>
                ...
                <p className="mb-0">
                  Klik ikon pensil untuk memilih metode pembayaran
                </p>
              </>
            )}
          </div>
          {selectedPaymentChannel && (
            <h5 className="mb-0 fw-bold text-primary">
              {paymentFee > 0 ? currencyFormat(paymentFee ?? 0) : "Gratis"}
            </h5>
          )}
        </div>
      </Card>
                        ...
                        return (
                          <Col md={6} key={index}>
                            <PaymentOption
                              ...
                              onClick={() =>
                                channel.active &&
                                handleSelectPaymentChannel(channel)
                              }
                            >
                              <PaymentIcon
                              ...
                              <div>
                                ...
                                <div>
                                  <small>
                                    {fee > 0 ? currencyFormat(fee) : "Gratis"}
                                  </small>
                                </div>
                              </div>
                            </PaymentOption>
  ...
  return (
    <div className="sticky-top" style={{ top: "5rem" }}>
      <Card body className="rounded-4">
        <Card.Title className="pb-3 mb-3 border-bottom">
        ...
        <div className="mt-3 text-center">
          <Link
            href={route("armada.show", { slug: carData.slug })}
      ...
      <Card className="mt-4" body>
        ...
        <p className="mb-0">
          Silahkan membaca dan memahami syarat dan ketentuan yang berlaku
          ...
          <Col md={8} className="relative">
            {isIdentityUnfilled && (
              <Alert
                variant="warning"
                className="mb-0"
                style={{ position: "sticky", top: "5rem", zIndex: 750 }}
              >
                ...
                <p className="mb-0">
                  ...
                  untuk melanjutkan transaksi dan pemesanan kendaraan rental dan
                  untuk keamanan Anda.
                </p>
              </Alert>
            )}
            ...
            <Card
              body
              className={twMerge("rounded-4", isIdentityUnfilled ? "mt-4" : "")}
            >
              <div className="gap-2 pb-3 mb-3 d-flex justify-content-between border-bottom align-items-center">
                ...
                <Button variant="danger" onClick={handleCancelCart}>
                  Batalkan
                </Button>
              </div>
